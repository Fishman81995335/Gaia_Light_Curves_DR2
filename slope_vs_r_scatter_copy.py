    ...
    with open(curve_file, 'r') as curve:
        with open(open_file, 'r') as result:
            ...
            for line in file_reader:
                if line[1] != 'slope':
                    line3 = next(curve_reader)
                    while line3[0] != line[0]:
                        line3 = next(curve_reader)
                        # Append values from light curve file source_id changes
                    ...
                    while line3[0] == line[0]:
                        ...
                        try:
                            ...
                            break
                    try:
                        if (np.amax(bp_val) > flux_min_1*np.amin(bp_val)) and (np.amax(bp_val) < flux_max_1*np.amin(bp_val)) \
                            and (np.amax(rp_val) > flux_min_1*np.amin(rp_val)) and (np.amax(rp_val) < flux_max_1*np.amin(rp_val))\
                                and count>=min_count:
                            color = color + ['g']
                            x = x + [float(line[1])]
                            y = y + [float(line[3])]
                        elif (np.amax(bp_val) > flux_min_2*np.amin(bp_val)) and (np.amax(bp_val) < flux_max_2*np.amin(bp_val)) \
                            and (np.amax(rp_val) > flux_min_2*np.amin(rp_val)) and (np.amax(rp_val) < flux_max_2*np.amin(rp_val))\
                                and count>=min_count:
                            color = color + ['b']
                            x = x + [float(line[1])]
                            y = y + [float(line[3])]
                        elif (np.amax(bp_val) > flux_min_3*np.amin(bp_val)) and (np.amax(bp_val) < flux_max_3*np.amin(bp_val)) \
                            and (np.amax(rp_val) > flux_min_3*np.amin(rp_val)) and (np.amax(rp_val) < flux_max_3*np.amin(rp_val))\
                                and count>=min_count:
                            color = color + ['r']
                            x = x + [float(line[1])]
                            y = y + [float(line[3])]
...
plt.savefig(master_file, dpi = 500)
plt.close(fig)
